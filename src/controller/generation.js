...
export async function generateWordsByTopic(req, res) {
  const { topic, groupId } = req.body;

  const content = `Подбери 20 английских слов по теме ${topic}. Составь из этих слов json со свойством "words", у которого значением будет массив объектов с полями englishWord, russianWord, englishExample, russianExample. Где englishWord - это анлийское слово, russianWord -  перевод на русский (если переводов несколько, то нужно вывести основные через запятую), englishExample - пример использования в предложении,  russianExample - перевод примера на русский. В примерах выдели само слово тегом <em></em>. Пришли только json без комментариев и без символов.`;
  try {
    ...
    const isArr = Array.isArray(data.words);
    ...
    const wordSet = data.words.map((item) => ({
      ...item,
      groupId,
      association: "",
      done: false,
